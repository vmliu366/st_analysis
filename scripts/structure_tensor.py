...
from matplotlib.colors import hsv_to_rgb  # pure function, no rendering
...
def construct_tensor(roi, sigma_g, sigma_w, truncate):
    ...
    w, _v = apply_gufunc(np.linalg.eigh, "(i,j)->(i),(i,j)", Jc)
    lam1 = w[..., 0]
    lam2 = w[..., 1]
    ...
    fxx_ = J[:, :, 0, 0]
    fxy_ = J[:, :, 0, 1]
    theta = 0.5 * da.angle((fyy_ - fxx_) + 1j * 2 * fxy_)
    mask = theta < 0
    adjusted = da.angle(da.exp(1j * theta) * da.exp(1j * np.pi))
    theta = da.where(mask, adjusted, theta)  # [0, pi)
...
def orientation_rgb_uint8(roi_np: np.ndarray, theta_np: np.ndarray, AI_np: np.ndarray) -> np.ndarray:
    # Hue encodes theta; saturation encodes AI; value encodes inverted intensity (as before)
    H = (theta_np / np.pi)  # [0,1)
    S = np.clip(AI_np, 0, 1)
...
def main():
    ...
    # Keep your patch-local normalization (fast), but seams may appear in full mosaics.
    ...
    np.save(out_arrays / "roi2.npy", patch_np)
    ...
    rgb8 = orientation_rgb_uint8(patch_np, theta_np, AI_np)
    np.save(out_arrays / "orientation_rgb.npy", rgb8)
    save_rgb_png(rgb8, out_figs / "orientation_polar.png")  # exact patch pixel size
